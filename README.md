...
[Air3 안경] ─┐                                        ┌─ [사무실 PC]
 broadcaster │  (1) 앞단(정적) = GitHub Pages           │  viewer
   (PWA)     │  (2) 시그널링 WSS + /ice-config          │  (북마크)
             │      = Cloudflare Workers (권장, 무료)   │
             │        └ 또는 Oracle VM (Node+Caddy)     │
             └─ (3) 미디어 릴레이 = Cloudflare ─────────┘
                    Realtime TURN (무료 1,000GB/월)
```

> **배포 방식 2가지** — 같은 프론트엔드에 시그널링 백엔드만 다릅니다.
> - **A. Cloudflare Workers (권장)**: `worker/` 폴더. VM·도메인·인증서·방화벽 불필요, 무료 HTTPS 자동. → 아래 "배포 A".
> - **B. Oracle VM**: `server.js` + `deploy/`. 평생 무료 실서버. → "배포 B".
...
├─ worker/                  ← 배포 A: Cloudflare Workers 시그널링(권장)
│  ├─ src/index.js          Worker + Durable Object(room 1:N) + TURN 발급
│  └─ wrangler.toml         Workers 설정(DO 바인딩/마이그레이션/vars)
├─ deploy/                  ← 배포 B: Oracle VM용
...
### 공통 준비: Cloudflare TURN 앱
대시보드 → Realtime → TURN Server → 앱 생성 (앱 이름 `air3`).
- **Turn Token ID (Key ID)**: `5e0680cdf6c5d42ebf16df4949f29d23` — 공개 식별자(비밀 아님).
- **API Token**: 시그널링 백엔드의 시크릿으로만 보관. git에는 절대 올리지 않음.
  ⚠️ 채팅/스크린샷에 노출된 적 있으면 **전체 작동 확인 후 Roll/재발급**.

---

## 배포 A — Cloudflare Workers (권장)

VM·도메인·인증서·방화벽 없이 무료 HTTPS 시그널링. `worker/` 폴더 사용.

```bash
cd worker
npm install
npx wrangler login                        # 브라우저 인증 (한 번)
npx wrangler secret put CF_TURN_API_TOKEN # Cloudflare TURN API 토큰 붙여넣기
npx wrangler deploy                       # 배포 → https://air3-signal.<서브도메인>.workers.dev
```

배포 후:
1. 출력된 `https://air3-signal.<서브도메인>.workers.dev` 주소를 복사.
2. `public/config.js`의 `SERVER_URL`을 그 주소로 교체 → GitHub에 push (Pages 자동 갱신).
3. 검증:
   ```bash
   curl https://air3-signal.<서브도메인>.workers.dev/ice-config   # hasTurn:true 면 성공
   ```

- 로컬 검증: `cd worker && npx wrangler dev --port 8787 --local` 후
  `PORT=8787 node ../tools/test-signaling.js` → 12/12 통과.
- 무료 등급에서 Durable Objects(SQLite 백엔드) + WebSocket Hibernation 사용.
